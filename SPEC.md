...
- **Audio Loading & Performance:**
  - **Requirement:** All sounds associated with the _currently active profile_ should be decoded and loaded into memory when the application loads or the profile is activated.
...
- **Stopping Sounds:**
  - **Panic Stop (Esc):** Pressing the `Escape` key immediately stops _all_ currently playing sounds. This function should _not_ be active when the search modal is open.
  ...
  - **Active Pad Trigger Behavior:** A profile setting determines what happens when a pad is triggered (via click or keyboard shortcut) while it is already playing:
    ...
    - **Note:** Clicking the track entry in the "Active Tracks" panel _always_ stops the sound immediately, regardless of this setting.
- **Multiple Sounds per Pad:**
  ...
  - **Playback Modes (`playbackType`):** A pad with multiple sounds must have one of the following modes:
    - `"sequential"` (Default): On trigger, always plays the first sound (`audioFileIds[0]`). _Future enhancement: Could play next sound in sequence on subsequent triggers or after completion._
...
- **Sound Assignment:**
  - **Drag and Drop:**
    - If a pad has 0 or 1 sound (`audioFileIds.length <= 1`), dragging an audio file onto it _replaces_ the existing sound(s) and updates the `padConfiguration` (`audioFileIds = [newId]`, `playbackType = 'sequential'`). The pad name defaults to the new filename without the extension.
    - If a pad has more than 1 sound (`audioFileIds.length > 1`), dragging an audio file onto it is _disabled_. Visual feedback should indicate this (e.g., different overlay).
...
- **Pad Actions:**
  ...
  - **Remove Sound (Direct Action):**
    - The visible "X" button and the `Delete`+click action _only_ function for pads with exactly _one_ sound (`audioFileIds.length === 1`).
    ...
    - If a pad has multiple sounds, clicking the "X" button or using `Delete`+click _opens the Pad Edit Modal_ instead of showing the confirmation.
...
- **Interaction within Modal:**
  ...
  - Pad/bank keyboard shortcuts are _disabled_ while the search modal is open.
  - Pressing `Escape` _only_ closes the search modal; it does not act as a panic stop. Other methods to close (e.g., click outside, close button) should also be considered.
...
- **Interaction:** Playing a pad directly does _not_ affect the armed sound(s); they remain armed.
...
- **Import:** Users select an exported JSON file.
  - The system parses it, creates a _new_ profile (handling potential name conflicts by appending `(n)`).
...
- **Implementation (Needs Rework):**
  ...
  - **Caching Strategy:**
    ...
    - **Audio Files:** Cache audio files aggressively (e.g., Cache-first). They are already stored in IndexedDB, but caching via the SW might improve initial load performance after the first fetch. Consider if SW caching is needed _in addition_ to IndexedDB. _Decision needed: Rely solely on IndexedDB for offline audio or also use Cache Storage via SW?_
