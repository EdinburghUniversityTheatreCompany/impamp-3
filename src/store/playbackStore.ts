...
interface PlaybackStoreState {
  ...
  actions: {
    ...
    updateTrackProgress: (key: string, progress: number, remainingTime: number) => void;
    setTrackFading: (key: string, isFading: boolean) => void;
    clearAllTracks: () => void;
  };
}

export const usePlaybackStore = create<PlaybackStoreState>((set) => ({ // Removed unused 'get' parameter
  ...
  actions: {
    ...
    addTrack: (key, initialState) => set((state) => {
    ...
    updateTrackProgress: (key, progress, remainingTime) => set((state) => {
    ...
    setTrackFading: (key, isFading) => set((state) => {
      const track = state.activePlayback.get(key);
      if (track) {
        const newMap = new Map(state.activePlayback);
...
export const useActivePlayback = () => usePlaybackStore((state) => state.activePlayback);
