...
// The audio-import register moved to its own module — it is a concurrency
// primitive, not storage, and it is the one rule here with no compiler behind
// it. Imported for this file's own deleters and re-exported below, because
// every existing caller reaches it through `@/lib/db` and the rule is
// repo-wide.
import {
  settleAudioImports,
  withAudioImportInProgress,
} from "./audioImportRegister";
export {
  beginAudioImport,
  settleAudioImports,
  withAudioImportInProgress,
