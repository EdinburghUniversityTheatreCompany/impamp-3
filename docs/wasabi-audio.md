...
or bucket — see `src/app/api/audio/audio.api.test.ts`. That file also covers
the deployment that sets none of the five variables, which is what every route
answers by default and what the seam above otherwise hides.

There is a second, coarser fake: `e2e-tests/fake-s3.js`, an S3-compatible
origin over real HTTP that the E2E run points the five `IMPAMP_S3_*` variables
at. It exists because an in-process fake cannot show that a presigned URL is
fetchable, that `Content-Length` comes back from a store rather than from the
client, or that a ranged GET returns the bytes proof of possession compares.
The end-to-end path — ask, PUT to the bucket, commit, list, download and
compare the bytes — runs against it in `e2e-tests/server-sync.spec.ts`,
including the rule that quota is charged from the size the bucket reports and
an over-size upload is deleted rather than kept. It deliberately does not
verify signatures; that is `sigv4.test.ts`'s job, below.
