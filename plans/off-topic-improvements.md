...
## The vanishing cleanup report also makes its own test flaky

The entry above is a UI complaint; it has a second cost. `OrphanedAudioPanel`s
"removes the unnamed rows and keeps the named one" failed once in a pre-commit
run and then passed 3/3 in isolation and in three full-suite runs afterwards,
which reads as noise and is not.

`reactPanel`s `press` settles by awaiting 40 `setTimeout(0)` turns inside one
`act` scope, and `setTimeout(0)` is clamped to about a millisecond — so on a
loaded machine those 40 turns span more than the 500 ms the panel waits before
re-scanning. The re-scan fires _inside_ the settle window, clears
`orphanCleanupResult`, and the assertion looks for a `data-testid` that is no
longer in the DOM. The test is asserting on a state the component deliberately
throws away, and whether it wins is a matter of machine load.

Fixing the clearing fixes both. Failing that, the test wants fake timers so the
500 ms is under its control rather than the scheduler's. Noticed while
converting the inline SVG icons.
